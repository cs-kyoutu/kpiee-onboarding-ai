...
function systemText(facts: ProjectFacts, spec: ReportSpec): string {
  return [
    ...
    '- 担当者の指示は「反映してから報告」が基本。上限・形式の制約に当たったときも、質問で止めずに',
    '  最も自然な代替（重要度の低い行との置き換え・別の節への移動・言い換え）を選んで',
    '  update_report_spec まで実行し、何をどう反映したかを短く報告する。判断が本当に割れるときだけ聞く。',
    '  指示を受けて何も反映しないターンを作らない（「反映されない」に見えるのが一番の失敗）。',
