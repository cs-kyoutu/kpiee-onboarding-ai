...
function buildQuestions(
  ...
  if (undetected.length > 0) {
    ...
    qs.push({
      ...
      // 見出しには「お答えいただきたいこと」だけを置く。どのファイルからどこへ運んでいるかは
      // 下の一覧（detail）に出ているので、見出しで並べ直すと、質問との間に話が1つ挟まって
      // 「結局、何を聞かれているのか」が読み取りにくくなる
      title: '毎月、どの時点のデータを貼っていらっしゃいますか。'
        + 'それが分かれば、kpiee 側でも同じ数字を再現できるようになります。',
  ...
  if (innerCopy.length > 0) {
    ...
    // 選択肢は1行ずつ渡す（1つの行に a〜c を詰めると項目の途中で折り返して読めない）。
    // 選択肢の記号に ①②③ は使わない。受領ファイルの名前が「①68期…」「②AMEX…」のように
    // 丸数字で始まる案件があり、設問の選択肢とファイル名が同じ記号で並ぶと読み違える。
    qs.push({
      priority: 'mid', kind: '手修正の確認',
      title: `数式でつながっていないのに、同じ値が並んでいる表が${innerCopy.length}組あります。`
        + 'これは手作業で数値を合わせていらっしゃいますか。'
        + '手修正の箇所が分かれば、kpiee でどこまで自動化できるかがはっきりします。',
      analysis: '値が同じで、かつ数式のつながりが見つからない表を、こちらで自動的に数えたものです。'
        ...
        + '下の a〜c のうち、近いものを1つお選びいただくだけで結構です。',
      detail: lines.concat(ordered.length > 5 ? [`ほか ${ordered.length - 5} シート`] : []),
      ask: ['a. 同じ様式を並べているだけで、数字はそれぞれ別に入っている。',
            'b. 手修正で合わせている。',
            'c. 一部が手修正（その場合は、手修正されているシートをお聞かせください）。'],
    });
  }
  ...
  // 移行の前提が欠けている状態なので、専用の問いとして優先度を上げて出す。
  {
    ...
    if (untraced.length > 0) {
      ...
      qs.push({
        ...
        title: `最終アウトプット ${untraced.length} シートについて、その数値の出どころが受領データの中に見当たりません。元になるファイルをご提供いただけますでしょうか。`,
  ...
  if (noKey.length > 0) {
    ...
    qs.push({
      ...
      // どのシートの話かは下の一覧に出るので、見出しは「何を教えていただきたいか」と
      // 「教えていただけないとどうなるか」だけにする
      title: '行を1つに決められる列（キー）を教えてください。'
        + 'このままでは kpiee に取り込んだときに、同じ行が重なってしまいます。',
  ...
  qs.push({
    ...
    // 「更新について」だけでは何を答えればよいのか分からない。聞きたい中身を見出しに出す
    title: '各ファイルの更新頻度・ご担当者と、今回いただいた以外に使っているファイルがあるかを教えてください。',
