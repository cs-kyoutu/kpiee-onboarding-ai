...
//
// なぜ必要か:
//   Excel の数式はファイルを跨げないため、ファイル間の関係はほぼ全て「値の一致による手コピー推定」
//   （confidence 0.55）になる。正しい推定もノイズも同じ確度で並ぶので、確認したい論点が埋もれる。
//   逆に、担当者が知っているつながり（外部リンク・ピボット・人手の転記）は自動検出できないと
//   図から消えてしまい、確認したい論点そのものが失われる。
//   そこで「人が知っている関係」を入力として受け取り、推定の確度を上下させる。
//
// 設計の要点:
//   - 数式由来の辺は絶対に変更しない。数式は事実であり、宣言で上書きしてよいものではない。
//     補正の対象は「ファイルを跨ぐ手コピー推定(copy)」だけ。
...
/** 未宣言のファイル間手コピーに掛ける減衰（0.55 → 約0.30、確度ラベルが「中」から「低」へ落ちる） */
const UNDECLARED_DAMP = 0.55;

/**
 * 確定済みブック関係をグラフへ重ね、ファイルを跨ぐ手コピー推定の確信度を補正する。
