...
// 生成器で直した「札の幅そろえ」だけを既存の HTML へ当てる使い捨てのパッチ。
//   npx tsx scripts/_patch-kyowa-tagw.ts <対象.html>
//
// 直す内容は relationsReport.ts の renderMiniSteps / CSS と同じ:
//   ・.mini-step .tag を塊ごとの固定幅（--tagw）にする
...
console.log(`CSS: ${cssHit ? '差し替えました' : '（すでに新しい指定でした）'} ／ 幅を入れた塊: ${groups} 件`);
